...
前置：Node ≥ 22；在仓库根目录已执行 `pnpm install`。

1. 在仓库根目录从 `.env.example` 复制并编辑 `.env`（填写 API Key 等）。
2. 在仓库根目录启动开发服务（server + web 同时启动）：
...
1. 在 `plugins/web-<id>/` 放一个 workspace 包（任何 HTTP server 技术栈都行：Next.js / Vue / SvelteKit / Hono / Express / 静态站 preview server / FastAPI 等）。`package.json` 必须包含 `scripts.start`、`scripts.dev`，以及 `goldpan.web.displayName`。
...
Goldpan 通过约定目录 `plugins/*` 支持外置采集器插件。`apps/server` 启动时会自动扫描该目录、加载每个插件的 `dist/index.js` 并注册到 `PluginRegistry`。
...
放入 `plugins/` 后，server 启动时自动发现和注册，无需修改任何接线代码。

### 手动安装第三方 plugin

设置页"插件 → 安装新插件"按钮目前 disabled（registry install 未实装）。手动安装步骤：

1. **放置目录**：把 plugin 包放到 `plugins/<name>/`，须含 `package.json` + `dist/index.js`（或可 build 出 `dist/index.js` 的 `tsconfig.json` + `src/index.ts`）。
...
在仓库根目录执行（会删除所有 `node_modules` 与 Next 构建缓存 `apps/web/.next`，然后按 lockfile 重装）：
