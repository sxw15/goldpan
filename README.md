<!--
  发版前请把 <OWNER> 替换成真实 GitHub owner/repo,例如:
    sed -i '' 's|<OWNER>|your-org/goldpan|g' README.md
  Linux 上去掉 sed 的空字符串参数: sed -i 's|<OWNER>|...|g' README.md
-->
...
[![GitHub Stars](https://img.shields.io/github/stars/<OWNER>?style=social)](https://github.com/<OWNER>)
...
```bash
git clone https://github.com/<OWNER>.git goldpan
cd goldpan/monorepo
pnpm install
...
```typescript
// monorepo/plugins/your-collector/src/index.ts
import {
...
放进 `monorepo/plugins/your-collector/`,跑 `pnpm -r build && pnpm dev`,server 启动时自动注册。完整 5 类插件的接口、`PluginContext`、`settingsContribution` 等细节由 `monorepo/docs/plugin-guide.md` 单独维护(陆续完善中)。

**怎么贡献插件**

两条路径:

1. **提 PR 进本仓库** — 路径 `monorepo/plugins/<name>/`,跟随 monorepo 一起发布。适合通用 / 高质量 / 想被官方维护的插件
2. **独立维护** — 发到 npm,包名遵循 `@goldpan/plugin-*` 约定,用户自己装到他们的 `monorepo/plugins/` 目录。适合实验性 / 小众 / 想自己掌控的插件
...
> 我在用一个叫 Goldpan 的自托管开源工具:把所有读到的文章 / 视频 / GitHub 项目自动消化成结构化知识库,最关键的是会做"增量比对" —— 同样话题刷十次,它只告诉你新内容。如果你也被收藏坟场困扰,强推:https://github.com/<OWNER>

⭐ **Star History**

[![Star History Chart](https://api.star-history.com/svg?repos=<OWNER>&type=Date)](https://star-history.com/#<OWNER>&Date)
...
- 💬 [GitHub Issues](https://github.com/<OWNER>/issues) / [Discussions](https://github.com/<OWNER>/discussions) — Bug 反馈 / 提案
